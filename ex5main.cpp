...
/*  // 1st test - dodawanie do pojemnika stalych, zmiennych, tymczasowych
  ...
  *c1 << 4 << 2+3;
  ...
  showTestResult(7, c1->indexOf(4, 4) == 4);

  // 8th test - metoda contains
  showTestResult(8, !c1->contains(-6));
  

  // 9th test - operacje na pojemniku w pojemniku
  for(int i=3;i>=0;i--)
    a.at(0) += i+1;

  bool t9 = a.at(0).size() == 4;
  int ttab9[] = {4, 3, 2, 1};
  for(int i=0;t9 && i<4;i++)
    t9 = t9 && (a.at(0).at(i) == ttab9[i]);
  showTestResult(9, t9);

  // 10th test - usuwanie z pojemnika
  a.at(0).remove(2);   // 4,3,1
  a.at(0).remove(1);   // 4,1
  a.at(0).remove(1);   // 4
  a.at(0).remove(0);   // empty
  showTestResult(10, a.at(0).isEmpty());

  // 11th test - dzialanie operatora przypisania
  *c2 = *c1 = a.at(0);
  showTestResult(11, *c1 == a.at(0));
  
  // 12th test - zwalnianie paieci
  try
    {
      delete c1;
      delete c2;
      showTestResult(12, true);
    }
  catch(...)
    {
